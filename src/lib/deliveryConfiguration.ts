...
type DeliveryConfiguration = Pick<StoreSettings,
  'tipo_taxa_entrega' | 'taxas_bairros' | 'taxa_bairro_padrao' |
  'bloquear_bairros_nao_atendidos' | 'taxa_entrega_fixa'
> & { delivery_regions_publication?: string };

export function hasConfiguredDeliveryRates(settings: DeliveryConfiguration): boolean {
  switch (settings.tipo_taxa_entrega) {
    case 'bairro':
      return Boolean(settings.taxas_bairros?.some((rate) => rate.ativo !== false && !rate.bloqueado))
        || (settings.taxa_bairro_padrao != null && Number.isFinite(settings.taxa_bairro_padrao) && settings.taxa_bairro_padrao >= 0)
        || settings.bloquear_bairros_nao_atendidos === false;
    case 'fixa':
      return typeof settings.taxa_entrega_fixa === 'number'
        && Number.isFinite(settings.taxa_entrega_fixa) && settings.taxa_entrega_fixa >= 0;
    case 'regiao':
    case 'bairro_regiao':
      return Boolean(settings.delivery_regions_publication);
    default:
      return false;
  }
}
