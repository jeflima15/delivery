      ...
      <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-12 lg:gap-4">
        {filteredBlocks.map((bloco) => {
          ...
          return (
            <div
              key={bloco._id}
              className={position === 'below_hero' && totalBlocks === 1 ? `${colSpanClass} lg:max-w-[280px] xl:max-w-[270px]` : colSpanClass}
            >
              ...
              {bloco.tipo_bloco === 'texto' && (
                <div className="flex h-full items-center justify-center rounded-[1.2rem] border border-[#e5e8e0] bg-white p-4 text-center shadow-[0_12px_26px_rgba(15,23,42,0.05)] dark:border-slate-700 dark:bg-slate-800">
                  <p className="text-[13px] font-bold italic leading-6 text-gray-700 dark:text-gray-300">
                    {bloco.descricao}
                  </p>
                </div>
              )}
