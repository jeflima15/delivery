...
export default function MasterApp() {
  const [path, setPath] = useState(() => normalizePath(window.location.pathname)); const [session, setSession] = useState<SessionResponse['account'] | null>(null); const [checking, setChecking] = useState(true); const [sessionError, setSessionError] = useState(''); const [expired, setExpired] = useState(false); const [platformName, setPlatformName] = useState('Pode Vir'); const [attention, setAttention] = useState(0); const [period, setPeriodState] = useState<PeriodKey>(() => (localStorage.getItem('master_period') as PeriodKey | null) || '30d'); const [toasts, setToasts] = useState<ToastMessage[]>([]);
