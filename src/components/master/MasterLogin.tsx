...
export default function MasterLogin({ expired, onSuccess }: Props) {
  ...
  return <main className="relative grid min-h-[100dvh] overflow-hidden bg-[#050914] p-4 text-white lg:grid-cols-[1.1fr_.9fr]">
    ...
    <section className="relative hidden flex-col justify-between p-12 lg:flex"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-emerald-400 text-slate-950"><ShieldCheck className="h-6 w-6"/></span><div><strong className="text-lg">Delivery Platform</strong><p className="text-xs text-slate-400">Centro de controle SaaS</p></div></div><div className="max-w-xl"><p className="text-sm font-semibold uppercase tracking-[.22em] text-emerald-400">Operação global</p><h1 className="mt-5 text-5xl font-bold leading-[1.08] tracking-tight">Decisões claras para uma plataforma saudável.</h1><p className="mt-6 text-lg leading-8 text-slate-400">Lojas, assinaturas, faturamento e atividade administrativa reunidos em um ambiente seguro e auditado.</p></div><p className="text-xs text-slate-600">Acesso exclusivo do proprietário da plataforma.</p></section>
