...
export default function ComboModal({
  ...
  const startingPriceCents = stages.reduce((total, stage) => {
  ...
  }, 0) || Number(product.preco_centavos ?? Math.round(product.preco * 100));

  const currentDisplayCents = totalUnitCents > 0 ? totalUnitCents : startingPriceCents;
    ...
    <div className="flex h-[100dvh] w-full max-w-3xl flex-col overflow-hidden bg-white sm:h-auto sm:max-h-[calc(100vh-48px)] sm:rounded-xl sm:shadow-2xl">
      <div className="min-h-0 flex-1 overflow-y-auto">
        <header className="relative border-b border-gray-200 bg-gray-50">
          ...
          <button type="button" onClick={onClose} className="absolute right-4 top-4 flex h-[30px] w-[30px] items-center justify-center rounded-full bg-gray-200 text-gray-500 shadow-sm cursor-pointer" aria-label="Fechar"><X className="h-4 w-4" /></button>
        </header>
        ...
        <div className="px-4 pb-5 pt-4 sm:px-5">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            {currentDisplayCents > startingPriceCents ? (
              ...
              <div className="divide-y divide-gray-100">
                {(stage.opcoes || []).map((configuredOption) => {
                  ...
                  return <div key={String(configuredOption.produtoId)} className={available ? '' : 'bg-gray-50 opacity-55'}>
                    <button type="button" disabled={!available} onClick={() => selectProduct(stageId, String(configuredOption.produtoId))} className="flex w-full items-center gap-3 px-4 py-3 text-left disabled:cursor-not-allowed cursor-pointer">
