...
export default function AcceptInvitation({ token }: { token: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const targetStore = data.store || { name: storeName.trim(), slug: cleanSlug };
      setActivatedStore(targetStore);
      setSuccess(true);
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'Não foi possível aceitar o convite e ativar a loja.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (success) {
    const finalStore = activatedStore || { name: storeName.trim(), slug: sanitizeSlugInput(slug) };
        ...
        <div className="w-full max-w-lg space-y-6 rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-8 sm:p-10 shadow-xl shadow-slate-900/5 text-center animate-in zoom-in-95 duration-300">
          ...
          <div className="space-y-2">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200 px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-emerald-700">
              <Sparkles className="h-3.5 w-3.5" /> Loja ativada com sucesso
            </span>
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-10 sm:py-16 flex items-center justify-center">
      ...
      >
          ...
          <div className="flex items-center gap-2.5 pb-2 border-b border-slate-100">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 font-medium">
                Informações para login do administrador
              </p>
            </div>
          </div>
        ...
        {/* Section 2: Identidade e Link da Loja */}
        ...
        <button
          disabled={
            submitting ||
            slugStatus === 'checking' ||
            (slug.trim().length >= 3 && slugStatus === 'unavailable')
          }
          type="submit"
        ...
        >
          {submitting ? (
            <>
              ...
              <span>Ativando loja e criando acesso...</span>
            </>
          ) : (
            <>
              <span>Ativar Loja e Acessar Painel</span>
              <ArrowRight className="h-5 w-5" />
            </>
          )}
        </button>
